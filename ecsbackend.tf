...
resource "aws_ecs_task_definition" "strapiecstaskdefinition1" {
    ...
    container_definitions = jsonencode([
        {
            ...
            image = "maheshryali/strapibackend:latest"
            essential = true
            portMappings = [
                {
                    containerPort = 1337
...
resource "aws_ecs_service" "ecs_service_strapi" {
    ...
    task_definition = aws_ecs_task_definition.strapiecstaskdefinition1.arn
    desired_count = 1
    ...
    depends_on = [ aws_ecs_task_definition.strapiecstaskdefinition1 ]
...
resource "aws_route53_record" "subdomain1" {
  ...
  name    = "maheshr-api.contentecho.in"  
  type    = "A"
  ttl     = 300
